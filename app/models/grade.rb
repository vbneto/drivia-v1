class Grade < ActiveRecord::Base
  ...
  def self.initialize_month_graph(student_grade,user)
    ...
    if user.is_parent? or user.is_school_administration? or user.is_professor?
      student_name = student_grade.blank? ? '' : student_grade.first.student_from_excel.student_name 
